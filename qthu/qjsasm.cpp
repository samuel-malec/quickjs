...
FunctionMetadata parse_metadata( const std::vector< Token >& tokens )
{
    ...
    for ( size_t i = 0; i < tokens.size(); i++ )
    {
        if ( tokens[ i ].type == Token::DIRECTIVE )
        {
            ...
            else if ( ( directive == ".locals" ) && i + 1 < tokens.size() && tokens[ i + 1 ].type == Token::NUMBER )
            {
                meta.var_count = std::stoi( tokens[ ++i ].value );
            } 
            else if ( directive == ".stack_size" && i + 1 < tokens.size() && tokens[ i + 1 ].type == Token::NUMBER )
            {
...
bytes wrap_function_bytecode( const bytes& bytecode_body, const FunctionMetadata& meta ) 
{
    ...
    {
        encode_atom( result, 0 ); // atom
        encode_leb128_i( result, 0 ); // scope_next (0 becomes -1 after decrement)
    ...
    // TODO: constant pool once we have it 
